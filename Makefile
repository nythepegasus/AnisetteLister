SHELL := /bin/bash
.PHONY: help ios update tvos clean
...
fakesign: build
	ldid -S$(APP)/Misc/$(APP).entitlements archive.xcarchive/Products/Applications/$(APP).app/$(APP)

ipa: fakesign
	mkdir Payload
	mkdir Payload/$(APP).app
	cp -R archive.xcarchive/Products/Applications/$(APP).app/ Payload/$(APP).app/
	zip -r $(APP).ipa Payload
	rm -rf Payload archive.xcarchive

clean:
	rm -rf $(APP).ipa $(APP).app.dSYM archive.xcarchive Payload
